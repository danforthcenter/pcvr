...
```{r}
sv<-bw.time(sv, plantingDelay = 0, phenotype="area", cutoff=10, timeCol="timestamp", group=c("barcode", "rotation"), plot=FALSE)
```
...
```{r}
sv_ag<-bw.outliers(df = sv_ag_withOutliers, phenotype="area", group = c("DAS", "genotype", "fertilizer"), plotgroup = c("barcode"), plot=FALSE)
pixels_per_cmsq <- 42.5^2 
sv_ag$area_cm2<-sv_ag$area / pixels_per_cmsq
