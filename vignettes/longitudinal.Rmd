...
![Credible intervals show the unrealistic homoskedasticity assumption](longitudinalPlots/homosked_brm.png)
...
![Linear sub models change variance over time, but do not fit this data well](longitudinalPlots/linear_brm.png)
...
![Spline sub models allow for taking heteroskedasticity into account without complex formulae](longitudinalPlots/spline_brm.png)
...
![Using a Gompertz formula for both growth and for increasing variance](longitudinalPlots/gompSubModel.png)
...
![LOO Information Criterion can be used to compare these models, with lower values indicating a better model fit](longitudinalPlots/pcvr_submodel_loo.png)
...
![Credible intervals from strong priors](longitudinalPlots/strongPrior.png)
...
![Using weak priors our mean lines make sense](longitudinalPlots/weakPrior.png)
...
```{r, eval=FALSE}
brms_ss<-growthSS(model="gompertz", form =  y~time|id/group,
...
Now we use `fitGrowth` to fit our models. Additional arguments can be passed to fitGrowth (see `?fitGrowth` for details), but here we only use those to specify details for the `brms` model and to run the `nlrq` model in parallel. Note that 500 iterations for the `brms` model is only to run a quick example, generally 2000 or more should be used and with more than 1 chain.
...
```{r, eval=FALSE}
brms_fit <- fitGrowth(brms_ss, iter = 500, cores = 1, chains = 1, 
...
![](longitudinalPlots/brmPlot1.png)

### Hypothesis testing

In linear regression the default null hypothesis ($\beta = 0$) can be useful as each beta past the intercept directly measures the effect of one variable. In non-linear regression we generally have more complicated model parameters and meaningful testing can be a little more difficult. In `pcvr` the `testGrowth` function allows for some hypothesis testing on model parameters for frequentist models. For Bayesian models the `brms::hypothesis` function is recommended due to its extreme versatility.

#### `testGrowth`

`testGrowth` takes the `growthSS` output used to fit a model, the model itself, and a list of parameters to test and performs likelihood ratio tests between the model and versions of itself where the selected parameters do not vary by group. The resulting p-value is broadly testing the null hypothesis that the groups have the same value for that parameter.

Here we see that for our `nls` model is statistically significantly improved by varying asymptote by group.

```{r}
testGrowth(nls_ss, nls_fit, test_pars = "A")$anova
```

Likewise for the 49th percentile in our `nlrq` model

```{r}
testGrowth(nlrq_ss, nlrq_fit, test_pars = "A")[["0.49"]]
```

And the same is shown in our `nlme` model.

```{r, warning=FALSE, message=FALSE}
testGrowth(nlme_ss, nlme_fit, test_pars = "A")$anova
```
...
# dput(hyp)
structure(list(Hypothesis = "((A_groupa))-(1.1*(A_groupb)) > 0", 
    Estimate = 16.7880224, Est.Error = 3.13518501598807, CI.Lower = 12.012705, 
    CI.Upper = 21.952505, Evid.Ratio = Inf, Post.Prob = 1, Star = "*"), row.names = c(NA, 
-1L), class = "data.frame")
```
...
![](longitudinalPlots/linearPlusLinear_fitGrowth.png)
...
![](longitudinalPlots/linearPlusLogistic_fitGrowth.png)
...
![](longitudinalPlots/linearPlusGAM_fitGrowth.png)
...
![](longitudinalPlots/linearPlusLinearPlusLinear_fitGrowth.png)
...
ss<-growthSS(model = "int + int", form=y~time|id/group, sigma="int + int",
             list("int1"=10, "changePoint1"=10, "int2"=20, #main model
                  "subint1"=10, "subchangePoint1"=10, "subint2"=10), # sub model
             df=simdf, type = "brms")
...
![](longitudinalPlots/intPlusInt_heteroskedastic_fitGrowth.png)
...
ss<-growthSS(model = "int + linear", form=y~time|id/group, sigma="int + linear",
                 list("int1"=10, "changePoint1"=10, "linear2A"=20,
                      "subint1"=10, "subchangePoint1"=10, "sublinear2A"=10),
                 df=simdf, type = "brms")
fit <- fitGrowth(ss, backend="cmdstanr", iter=500, chains=1, cores=1)
plot <- growthPlot(fit=fit, form=ss$pcvrForm, df = ss$df, timeRange = 1:40)
```

![](longitudinalPlots/intPlusLinear_heteroskedasticIntPlusLinear_fitGrowth.png)
...
![](longitudinalPlots/intPluslogistic_intPlusGAMSubModel.png)
...
![](longitudinalPlots/distPlot_vigPlot.png)
