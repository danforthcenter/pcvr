...
Finally we can use the spline sub models. The spline model does a very good job of fitting the data due to the natural flexibility of polynomial functions. Again this added accuracy comes at the cost of taking longer for the model to fit.
...
For this data there are several reasonable looking curves. In some cases you might decide to fit a model using a different sub model than those offered in `pcvr`, which may work similarly to this example. Here we parameterize a non-linear formula for sigma. The downside of this is that it is more complicated to write this less flexible model while the upside is that it makes hypothesis testing about the variance more straightforward, make future data prediction more stable, and it has fewer parameters to estimate than a default spline submodel (thus it is significantly faster). 
...
form_b <- bf(y ~ A*exp(-B*exp(-C*time)),
           nlf(sigma ~ subA*exp(-subB*exp(-subC*time))), # nlf() is used to add additional non-linear formulas
           A+B+C+AA+BB+CC ~ 0+group, # no intercepts, everything modeled by groups
           autocor = ~arma(~time|sample:group,1,1), nl = TRUE )


fit_gomp_gompSubModel <- brm(form_b,
                        family = student, prior = prior1, data = gomp,
                        iter = 1000, cores = 4, chains = 4, backend = "cmdstanr", silent = 0,
                        control = list(adapt_delta = 0.999,max_treedepth = 20),
                        init = 0 ) # 200 seconds vs 4800 seconds for spline sub model
brmPlot(fit_gomp_gompSubModel, form = y~time|id/group, df = gomp)
```
...
```{r, eval = FALSE}
print(load("splineFit.rdata"))
print(load("gompSubModel.rdata"))
print(load("homoSubModel.rdata"))
print(load("linearSubModel.rdata"))
# loo IC comparison
loo_spline <- add_criterion(fit, "loo")
loo_homo <- add_criterion(fit_h, "loo")
loo_linear <- add_criterion(fit_l, "loo")
loo_gomp <- add_criterion(fit_gomp_gompSubModel, "loo")
...
#loo(fit, fit_gomp_gompSubModel, fit_h)
...
ggsave("pcvr_submodel_loo.png", bg="#ffffff", width = 8, height = 6, dpi = 300)
```

![LOO Information Criterion can be used to compare these models, with lower values indicating a better model fit](pcvr_submodel_loo.png)
...
ss <- growthSS(model = "linear", form = y~time|id/group, sigma = "spline",
...
#ggsave("strongPrior.png", bg="#ffffff", width = 9, height = 6, dpi = 300)
```

![Credible intervals from strong priors](strongPrior.png)
...
brmPlot(fit, form = ss$pcvrForm, df=ss$df)+
 coord_cartesian(ylim = c(0,100)) # ggsave("weakPrior.png", bg="#ffffff", width = 9, height = 6, dpi = 300)
```

![Using weak priors our mean lines make sense](weakPrior.png)

As you can see the weak priors moved to meet our data and now we have useful posterior distributions.
...
0.5*( M1/(1+exp(-k1(x-a))) + M2/(1+exp(-k2(x-b))))

Where M1 and M2 are the asymptotics (the plateaus), a and b are the x values for the centers of the slopes and k1 and k2 are connected to the steepness of the curves.

## Double sigmoid (logistic)

```{r}
set.seed(345)
ds <- growthSim("logistic", n=20, t=35, params = list("A"=200, "B" = 13, "C" = 3))
ggplot(ds,aes(time, y, group=interaction(group,id)))+ 
  geom_line(aes(color=group))+labs(title="Logistic")
ds2 <- growthSim("logistic", n=20, t=35, params = list("A"=200, "B" = 13, "C" = 3))

res<-list()
for(group in unique(ds2$group)){
  sub<- ds2[ds2$group == group,]
  for(id in unique(sub$id)){
    ind <- sub[sub$id == id, ]
    plateau <- ds[ds$group ==group & ds$id == id & ds$time == 35, ]
    ind$time <- ind$time + plateau$time
    ind$y <- ind$y+plateau$y
    res[[paste0(group,id)]]<-ind
  }
}
ds2<-do.call(rbind, res)
rownames(ds2)<-NULL
ds <- rbind(ds, ds2)

ggplot(ds,aes(time, y, group=interaction(group,id)))+ 
  geom_line(aes(color=group))+labs(title="Double Sigmoid (logistic)")

prior_ds <- prior(gamma(2,0.1), class="nu", lb=0.001)+
  prior(lognormal(log(130), .25),nlpar = "A1", lb = 0) +
  prior(lognormal(log(12), .25), nlpar = "B1", lb = 0) + 
  prior(lognormal(log(1.2), .25), nlpar = "C1", lb = 0)+
  prior(lognormal(log(300), .25),nlpar = "A2", lb = 0) +
  prior(lognormal(log(30), .25), nlpar = "B2", lb = 0) + 
  prior(lognormal(log(1.2), .25), nlpar = "C2", lb = 0)

form_b <- bf(y ~ A1/(1+exp((B1-time)/C1)) + ((A2-A1) /(1+exp((B2-time)/C2))) ,
           sigma ~ 1,
           A1+B1+C1+A2+B2+C2 ~ 1, 
           autocor = ~arma(~time|sample:group,1,1), nl = TRUE)

fit_ds <- brm(form_b, family = student, prior = prior_ds, data = ds,
                        iter = 600, cores = 4, chains = 4, backend = "cmdstanr", silent = 0,
                        init = 0)

probs <- seq(from=99, to=1, by=-2)/100
avg_pal <- viridis::plasma(n=length(probs))
df_test <- data.frame("treatment"="a",time=1:70,sample="new1")
df_pred <- predict(fit_ds,df_test,probs=probs)
test <- cbind(df_test,df_pred)

ggplot(test,aes(time,Estimate))+
  lapply(seq(1,49,2),function(i) geom_ribbon(aes_string(ymin=paste("Q",i,sep = ""),ymax=paste("Q",100-i,sep = "")),fill=avg_pal[i],alpha=0.5))+
  geom_line(data=ds,aes(time,y,group=id),color="gray20")+
  ylab(~~Area~(cm^2))+
  xlab("Time")+
  theme_light()+
...
## Double sigmoid (gompertz)

```{r}
#* simulate double sigmoid data
set.seed(345)
ds <- growthSim("gompertz", n=20, t=35, params = list("A"=200, "B" = 20, "C" = 0.2))
ds2 <- growthSim("gompertz", n=20, t=50, params = list("A"=100, "B" = 13, "C" = 0.1))
...
for(group in unique(ds2$group)){
  ...
  for(id in unique(sub$id)){
    ind <- sub[sub$id == id, ]
    plateau <- ds[ds$group ==group & ds$id == id & ds$time == max(ds$time), ]
    ind$time <- ind$time + plateau$time
    ind$y <- ind$y+plateau$y
    res[[paste0(group,id)]]<-ind
  }
...
ggplot(ds,aes(time, y, group=interaction(group,id)))+ 
  geom_line(aes(color=group))+labs(title="Double Sigmoid (gompertz)")

prior_ds_1 <- prior(gamma(2,0.1), class="nu", lb=0.001)+
  prior(lognormal(log(130), .25),nlpar = "A1", lb = 0) +
  prior(lognormal(log(12), .25), nlpar = "B1", lb = 0) +
  prior(lognormal(log(1), .25), nlpar = "C1", lb = 0)+
  prior(lognormal(log(300), .25),nlpar = "A2", lb = 0) +
  prior(lognormal(log(50), .25), nlpar = "B2", lb = 0) +
  prior(lognormal(log(1), .25), nlpar = "C2", lb = 0)

prior_ds_2 <- prior(gamma(2,0.1), class="nu", lb=0.001)+
  prior(lognormal(log(130), .25),nlpar = "A1", lb = 0) +
  prior(lognormal(log(12), .25), nlpar = "B1", lb = 0) +
  prior(lognormal(log(1), .25), nlpar = "C1", lb = 0)+
  prior(lognormal(log(300), .25),nlpar = "A2", lb = 0) +
  prior(lognormal(log(150), .25), nlpar = "B2", lb = 0) +
  prior(lognormal(log(1), .25), nlpar = "C2", lb = 0)

# y ~ A1 * exp(-B1 * exp(-C1*x)) + (A2-A1) * exp(-B2 * exp(-C2*x))
# adding -B1 into the final growth segment is helpful for having interpretable
# B2, but it probably slows things down a lot.
form_b_1 <- bf(y ~ A1 * exp(-B1 * exp(-C1*time)) + (A2-A1) * exp(-B2 * exp(-C2*(time-B1))),
           sigma ~ 1,
           A1+B1+C1+A2+B2+C2 ~ 1,
           autocor = ~arma(~time|sample:group,1,1), nl = TRUE)

form_b_2 <- bf(y ~ A1 * exp(-B1 * exp(-C1*time)) + (A2-A1) * exp(-B2 * exp(-C2*(time))),
           sigma ~ 1,
           A1+B1+C1+A2+B2+C2 ~ 1,
           autocor = ~arma(~time|sample:group,1,1), nl = TRUE)


fit_ds_1 <- brm(form_b_1, family = student, prior = prior_ds_1, data = ds,
                        iter = 600, cores = 4, chains = 4, backend = "cmdstanr", silent = 0,
                        init = 0)

fit_ds_2 <- brm(form_b_2, family = student, prior = prior_ds_1, data = ds,
                        iter = 600, cores = 4, chains = 4, backend = "cmdstanr", silent = 0,
                        init = 0)
...
probs <- seq(from=99, to=1, by=-2)/100
avg_pal <- viridis::plasma(n=length(probs))
df_test <- data.frame("treatment"="a",time=1:max(ds$time),sample="new1")
df_pred <- predict(fit_ds_2,df_test,probs=probs)
test <- cbind(df_test,df_pred)
...
C2 = 0.1
x<-1:100
y <- A1 * exp(-B1 * exp(-C1*x)) + (A2-A1) * exp(-B2 * exp(-C2*(x-B1)))
plot(x,y,type="l")
```
