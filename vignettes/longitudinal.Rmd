...
date: "`r Sys.Date()`"
vignette: >
  %\VignetteIndexEntry{Longitudinal Growth Modeling Options}
...
```{r}
#| fig.alt: >
#|   Example of how much variance may change over an experiment
set.seed(345)
...
#|   being unrealistic and of several other sub models that may be more useful
...
```{r, eval=FALSE}
#| fig.alt: >
#|   Posterior predictive plot of a non linear growth model assuming homoskedasticity
...
```{r, eval=FALSE}
#| fig.alt: >
#|   Example of an inappropriate sub model for variance, in this case linearly increasing over time.
fit_l <- fitGrowth(ss,
...
```{r, eval=FALSE}
#| fig.alt: >
#|   Example of using splines to measure variance over time, which gives a good fit to the
#|   data but makes hypothesis testing about the variance very difficult
fit_s <- fitGrowth(ss,
...
```{r, eval=FALSE}
#| fig.alt: >
#|   Example of using another sigmoid model as the variance sub model, which here balances
#|   the good fit of the splines with the easy interpretation of the other sub models.
fit_g <- fitGrowth(ss,
  iter = 2000, cores = 4, chains = 4, silent = 0,
  control = list(adapt_delta = 0.999, max_treedepth = 20)
)
...
```{r, message = FALSE}
#| fig.alt: >
#|   Various possible sub models compared to the trend we get from our data. There are
#|   near infinite combinations but simplicity is beneficial.
draw_gomp_sigma <- function(x) {
  return(23 * exp(-21 * exp(-0.22 * x)))
}
...
```{r, eval = FALSE}
#| fig.alt: >
#|   Comparing LOO Information Criterion as one option for picking the best model parameterization.
#|   Here splines and the gompertz sub model are most appropriate.
...
```{r, eval = FALSE}
#| fig.alt: >
#|   Example of using very strong priors and how that can impact your model.
set.seed(345)
...
```{r, eval = FALSE}
#| fig.alt: >
#|   Example of using mildly informative priors, per the pcvr defaults and how that impacts your
#|   model fit.
...
```{r}
#| fig.alt: >
#|   Example of the default prior distributions as PDFs.
...
```{r}
#| fig.alt: >
#|   Example of the default prior distributions as PDFs with the growth data they imply shown.
...
Multi value traits can be modeled in the same ways using the `mvSS` function.

For the most part `mvSS` has the same arguments as `growthSS` but with the addition of a `spectral_index` argument, which can be "none" or any of the indices listed in the `PlantCV` [docs](https://plantcv.readthedocs.io/en/latest/spectral_index/). If a spectral index is specified then any truncation of the response variable is handled for you and the model family is changed to skew_normal (if using the `brms` backend).

Additionally, since there are generally fewer options for working with multi value traits the `mvSS` function can use non-longitudinal data, as in the example below, where the formula specifies `bins` given `value` per bin predicted by `group`.
...
Now we have all the essential model components in the `..._ss` objects. Since we specified a logistic model we have three parameters, the asymptote (`A`), the inflection point (`B`), and the growth rate (`C`). In the `brms` option our sub model uses a GAM and does not add more parameters. Note that in practice gompertz models tend to fit real data better than logistic models, but they can be more difficult to fit using the frequentist backends.
...
```{r}
#| fig.alt: >
#|   Checking the grouping of our model formulas with a line plot. Looking to make sure that
#|   each line is only identifying a single plant, which it is here.
...
Now we use `fitGrowth` to fit our models. Additional arguments can be passed to fitGrowth (see `?fitGrowth` for details), but here we only use those to specify details for the `brms` model. Note that 500 iterations for the `brms` model is only to run a quick example, generally 2000 or more should be used and with more than 1 chain.
...
```{r, warning=FALSE, message=FALSE}
#| fig.alt: >
#|   Examples of growth plots generated from growthPlot for each model backend besides brms.
...
As alluded to previously, using the `brms` backend segmented models are possible and can be specified using "model1 + model2" syntax, where the "+" specifies a changepoint where the process shifts from model1 to model2. As many models as you need to combine can be combined in this way, but more than 2 segments is likely to be very slow and can become more difficult to fit well. If you add many segments together then consider editing the `growthSS` output to set stronger priors on any changepoints that are in the same vicinity. Since the changepoints are parameterized they require priors and can be included in any hypothesis tests on the model.
...
![Plots of posterior prediction and posterior distributions over time for several models.](distPlot_vigPlot.png)

## Other Resources

The supported models in `pcvr` are meant to lower the barrier to entry for complex models using `brms` and `Stan`. While there are lots of options using these growth models there are many many more options using `brms` directly and still more is possible programming directly in `Stan`. If you take an interest in developing more and more nuanced models then please see the [`brms` documentation](https://github.com/paul-buerkner/brms), [`Stan` documentation](https://mc-stan.org/users/documentation/), or [stan forums](https://discourse.mc-stan.org/). For models that you think may have broader appeal for high throughput plant phenotyping please raise an issue on [github](https://github.com/danforthcenter/pcvr) and help the DDPSC data science team understand what the desired model is/where existing options fall short for your needs.
