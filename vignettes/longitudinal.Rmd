...
```{r, eval=FALSE}
brms_ss<-growthSS(model="logistic", form =  y~time|id/group,
...
```{r}
testGrowth(fit = nls_fit, test = list("A1 - A2 *1.1",
                                       "(B1+1) - B2",
                                       "C1 - (C2-0.5)",
                                      "A1/B1 - (1.1 * A2/B2)"))
```
...
```{r}
testGrowth(fit = nlme_fit, test = list("(A.groupa / A.groupb) - 0.9",
                                  "1 + (B.groupa - B.groupb)",
                                  "C.groupa/C.groupb - 1"))
```
