...
For this vignette we will work with simulated data based off of mini rhyzotron data collected from
Fischer farms in the Fall of 2023.
...
sv_from_mv <- function(df, theta){ # note this should also return mean/median/std
  ...
  out <- aggregate(as.formula(paste0("value ~ ", paste(metaCols, collapse="+"))), data = df, sum, na.rm=TRUE)
...
ex <- do.call(rbind, lapply(1:20, function(rep){
  ...
  breaks <- h$breaks[-1]
  counts <- h$counts
  data.frame(rep = as.character(rep),
...
df <- do.call(rbind, lapply(seq_len(nrow(parameters)), function(time){
  ...
  x <- rRhyzoDist(n = n_total_pixels, theta = pars$theta,
                  u1_max = pars$u1_max,
...
df$rep <- factor(df$rep, levels = 1:length(unique(df$rep)), ordered = TRUE)
sv <- sv_from_mv(df)
```
...
df2 <- do.call(rbind, lapply(1:10, function(rep){
  ...
  for (time in seq_len(nrow(parameters))) {
    ...
    breaks <- h$breaks[-1]
    counts <- h$counts
    dList[[time]]<- data.frame(rep = as.character(rep),
                               time = as.character(time),
                               value = counts, label = breaks,
                               trait = "x_frequencies")
    add_area <- 0
...
df2$rep <- factor(df2$rep, levels = 1:length(unique(df2$rep)), ordered = TRUE)
sv2 <- sv_from_mv(df2)
```
...
getPeaks <- function(d = NULL, intensity = 20, duration = 3) {
  ...
  if(length(binwidth) > 1){stop("label column should have constant bin size")}
...
peak_df$rep <- unlist(lapply(names(d), function(nm) {strsplit(nm, "[.]")[[1]][[1]]}))
peak_df$time <- unlist(lapply(names(d), function(nm) {strsplit(nm, "[.]")[[1]][[2]]}))
```
