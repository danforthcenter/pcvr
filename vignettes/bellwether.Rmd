...
sv <- read.pcv(paste0(base_url, "pcv4-single-value-traits.csv"),
  reader = "fread"
)
```
...
example <- read.pcv("prohibitivelyLargeFile.csv",
  filters = list("trait in area_pixels, area_above_reference_pixels, area_below_reference_pixels",
                 "sample in default")
)
```
...
phenosAndMeta <- read.pcv.3(file = paste0(base_url, "pcv3Phenos.csv"), metaCol = "meta",
...
```{r}
out <- bw.outliers(df = sv_ag_with_outliers, phenotype = "area_pixels",
...
ggplot(water[water$weight_after != -1, ], aes(x = DAS,
...
```{r}
pcvBox(sv_ag[sv_ag$genotype == "MM" & sv_ag$DAS == 15, ], x = "fertilizer", y = "area_cm2",
       compare = "0", showPoints = TRUE) +
  labs(y = expression("Area" ~ "(cm"^2 ~ ")"), fill = "Soil Fertilizer") +
...
```{r}
pcvBox(cp[cp$DAS == 19, ], x = "fertilizer", y = "area_cm2_csum", showPoints = TRUE) +
...
For the purposes of this vignette we will only consider the default Student T model family, for examples of other model families see the [Advanced Growth Modeling tutorial on github](https://github.com/danforthcenter/pcvr/tree/main/tutorials/pcvrTutorial_advancedGrowthModeling) or the longitudinalGrowth vignette.
...
```{r, eval=FALSE}
print(load(url(paste0(base_url, "brmsFits.rdata"))))
from3to25 <- list(fit_3, fit_5, fit_7, fit_9, fit_11, fit_13,
...
```{r}
mo17_sample <- hue_wide[hue_wide$genotype == "Mo17" & hue_wide$DAS > 18 & hue_wide$fertilizer == 100,
                        grepl("hue_freq", colnames(hue_wide))]
b73_sample <- hue_wide[hue_wide$genotype == "B73" & hue_wide$DAS > 18 & hue_wide$fertilizer == 100,
                       grepl("hue_freq", colnames(hue_wide))]
...
```{r}
pcadf(hue_wide, cols = "hue_frequencies", color = c("genotype", "fertilizer"),
      trace = "DAS", returnData = FALSE)
```
...
```{r}
plots <- pcadf(hue_wide, cols = "hue_frequencies", color = c("genotype", "fertilizer"),
               trace = "DAS", returnData = FALSE, facet = c(1, 2, 3, 4))
