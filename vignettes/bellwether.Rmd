...
library(pcvr) # devtools::load_all("/home/jsumner/Desktop/stargate/fahlgren_lab/pcvr")
...
Bellwether data can also be read in with `read.pcv.bw` which is a wrapper around `read.pcv` which attempts to do several common tasks related bellwether data joining internally. This should be considered experimental as it is based on plantCV version 3 and earlier outputs and bellwether experiments can take many formats and this has only been considered with a few datasets.
...
```{r, eval=F}
net<-pcv.net(EMD$data, meta = c("fertilizer", "genotype", "DAS"))
```

```{r, eval=F}
quantile(net$edges$emd, 0.9)
net.plot(net, fill = "DAS", shape = c("genotype"), size=5, edgeFilter=1.5)
```
