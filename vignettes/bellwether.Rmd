...
```{r}
sv<-bw.time(sv, plantingDelay = 0, phenotype="area", cutoff=10, timeCol="timestamp", group=c("barcode", "rotation"), plot=TRUE)
head(sv)
dim(sv)
```
...
```{r}
phenotypes <- colnames(sv)[c(19:35, 43:45)]
...
```{r}
sv_ag<-bw.outliers(df = sv_ag_withOutliers, phenotype="area", group = c("DAS", "genotype", "fertilizer"), plotgroup = c("barcode"))
dim(sv_ag)
```
...
```{r}
test<-pwue(sv_ag, water, "area", "DAS", c("barcode"))
...
frem(sv_ag, des=c("genotype", "fertilizer"),
     phenotypes=c("area", "area_above_reference", "area_below_reference", "color_chip_size", 
                  "convex_hull_area", "convex_hull_vertices", "ellipse_angle", 
                  "ellipse_eccentricity", "ellipse_major_axis", "ellipse_minor_axis", 
                  "height", "height_above_reference", "height_below_reference", 
                  "horizontal_reference_position", "hue_circular_mean", "hue_circular_std", 
                  "hue_median", "perimeter", "solidity", "width"),
...
sv_ag$height_cm <- sv_ag$height/42.5
```
...
```{r}
ggplot(sv_ag[sv_ag$DAS==18,], aes(x = fertilizer, y=hue_circular_mean, fill = fertilizer))+
