...
sv_ag_withOutliers<-aggregate(form, data=sv, mean, na.rm=T)
dim(sv_ag_withOutliers)
```


The `bw.outliers` function can be used to remove outliers relative to a phenotype using cook's distance. Here just over 4 percent of data are removed as outliers. The plot shows removed data points in red.

```{r}
sv_ag<-bw.outliers(df = sv_ag_withOutliers, phenotype="area.pixels", group = c("DAS", "genotype", "fertilizer"), plotgroup = c("barcode"))
dim(sv_ag)
```
...
sv_ag$height_cm <- sv_ag$height.pixels/42.5
```
...
```{r}
ggplot(sv_ag, aes(x=DAS, y=area_cm2, group=interaction(genotype, fertilizer, lex.order=T),
                                color = genotype))+
  facet_wrap(~factor(fertilizer, levels = c("0", "50", "100")))+
  geom_smooth(method="loess", se=T, fill = "gray90")+
...
area_res_t <- conjugate(s1 = mo17_area, s2= B73_area, method="t", plot=T, rope_range = c(-5,5))
```
...
This should probably reduce the dimensions of the dataset, something like katie's method with a mean and SE for each.
AKA just compare averages.




```{r}
rt<-relativeTolerance(sv_ag, phenotypes = c("area_cm2", "height_cm"), grouping = c("fertilizer", "genotype", "DAS"), control = "fertilizer", controlGroup="100", method =  c("proportion", "difference", "zscore"), naTo0=T)
...
```{r}
ggplot(sv_ag[sv_ag$DAS==18,], aes(x = fertilizer, y=hue_circular_mean.degrees, fill = fertilizer))+
...
p<-pcv.joyplot(hue_wide[hue_wide$fertilizer==0 & hue_wide$DAS %in% c(7,12,18),], index = "hue_frequencies", group=c("DAS", "genotype"),
               trait=NULL,compare=NULL)
p+scale_fill_gradientn(colors = scales::hue_pal(l=65)(360))+scale_y_discrete(limits=c("7", "12", "18"))
```
...
hue_res_ln <- conjugate(s1 = mo17_sample, s2= B73_sample, method="lognormal",
              plot=T, rope_range = c(-20,20), hypothesis="equal")
```
...
pcadf(hue_wide, cols = "hue_frequencies", color = "genotype", returnData=F)+
  facet_wrap(~factor(fertilizer, levels=c("0", "50", "100")))
```
