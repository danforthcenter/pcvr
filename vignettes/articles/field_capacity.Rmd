...
n_plants <- 20
ex <- data.frame(id = paste0("Plant ", rep(seq_len(n_plants), each = days * 2)),
           time = rep(c(1:days - 0.5, 1:days + 0.1), times = n_plants),
...
pheno <- data.frame(
  id = paste0("Plant ", rep(seq_len(n_plants), each = days)),
  time = rep(seq(1, days, 1), times = n_plants),
  DAS = rep(seq(1, days, 1), times = n_plants),
  variable = "area",
  value = unlist(lapply(seq_len(n_plants), function(i) {
...
First we'll have to widen our data to fit the function's inputs.

```{r}
exw <- reshape2::dcast(ex, DAS + id ~ variable, value.var = "value")
```
