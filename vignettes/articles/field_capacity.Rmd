...
WUE might also be considered as the amount of water used to sustain a given biomass.
...
```{r}
ggplot(ex[ex$id %in% paste0("Plant ", 1:4), ], aes(x = DAS, y = value, color = variable)) +
  facet_wrap(~id) +
  scale_fill_manual(values = c("gray80", "antiquewhite")) +
  coord_cartesian(ylim = c(0.9 * round(min(ex$value)), 1.1 * round(max(ex$value))),
                  expand = FALSE) +
