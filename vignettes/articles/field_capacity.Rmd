...
watering_conditions <- list(
  "constant" = list("before" = NA,
                    "after" = rep(25, days)),
  "target weight" = list("before" = round(seq(90, 20, length.out = days)),
                         "after" = rep(100, days)),
...
models <- c("logistic", "logistic", "exponential", "logistic", "logistic", "logistic")
...
Fpheno <- list(A = 200, B = 12, C = 1) # "normal" sigmoid
...
Fwater <- rep(25, days) # constant watering
...
We'll look at an example rep of each group to get started (except `F`, we'll use that to test intuition later).

```{r}
ggplot(rb[rb$id == "id_1" & rb$group != "F", ], aes(x = time, y = pWUE, group = id)) +
  facet_wrap(~group) +
  geom_point() +
  geom_line() +
  pcv_theme()
...
ggplot(rbd,
  ...
  geom_point() +
  geom_line() +
  labs(title = "Intuition paid off, D follows the expected pattern") +
  pcv_theme()
```
...
ggplot(rbd,
  ...
  geom_point(aes(y = pheno_diff, color = "Phenotype (numerator)")) +
  geom_line(aes(y = pheno_diff, color = "Phenotype (numerator)")) +
  labs(title = "Water amount (weight) is very noisy here",
       y = "Pheno Diff and Water Diff", x = "Time") +
  pcv_theme() +
  theme(legend.position = "bottom")
```
...
After seeing the pWUE trend that `D` and `E` exhibit, what should happen with `F`?

Looking at the phenotype for group `F` it is a very abrupt sigmoid, almost like a step function. Since there is a long starting period of no major change followed by a few measurements where the phenotype shoots up towards its asymptotic limit we can probably expect to see a more dramatic spike in the middle of our pWUE trend. The "curve" should be steeper and resemble the cumulative density function (CDF) of the corresponding logistic curve.

```{r}
ggplot(rb[which(rb$group == "F" & rb$total_water > 0), ],
       aes(x = time, y = pWUE, group = id)) +
  geom_point() +
  geom_line() +
  labs(title = "The effect of very fast phenotype changes") +
  pcv_theme()
```

It looks about how we'd expect. The "peak" is much higher since the change in the phenotype happens significantly faster and water in this case is constant. 

In these examples rate based pWUE is mostly driven by the growth curve of the phenotype (area). There are differences in watering too, but these measures are at least as dependent on the phenotype as on anything else. 

```{r}
ggplot(rb[which(rb$id %in% paste0("id_", 1:3) & rb$total_water > 0), ],
       aes(x = time, y = pWUE, group = id)) +
