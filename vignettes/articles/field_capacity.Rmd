...
In this article we'll use simulated data, but the `pcv.water` function may be helpful for you reading in JSON Lemnatech metadata.
...
The previous plot would show us if anything were very strange in our data but it does not tell us whether or not we had the type of watering that we expected. For example, if we had a drought treatment we can't tell from that plot whether or not the soil in each pot was in the range of drought stress that we were aiming for. To understand that we need to use field capacity and compare against our target values.

Target values for field capacity should be decided as part of experimental design and will be returned by the phenotyping core in a format consistent with the simulated data below.

```{r}
d <- data.frame(
  experiment = "SIM001",
  variable = c("carriage_saucer", "dry_weight", "saturated_weight", rep(
    c("target_weight", "field_capacity"), each = 40
    )),
...
It generally makes sense to monitor field capacity of pre and post watering weights to make sure that the watering is keeping your soil in line with what your design requires.
