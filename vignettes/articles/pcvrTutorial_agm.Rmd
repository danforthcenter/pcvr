...
#|   being more prone to overfitting and fewer knots being less accurate for the observed data.
...
#| fig.alt: >
#|   Checking the grouping of data using a ggplot before making a model. We are looking to see that
#|   lines follow individual plants over time, here they do.
...
```{r}
#| fig.alt: >
#|   Comparing several parameterizations of sigma against the trend we see in our data.
#|   Homoskedastic is a poor fit.
ggplot(sigma_df, aes(x = time, y = y, group = group)) +
...
#| fig.alt: >
#|   Comparing several parameterizations of sigma against the trend we see in our data.
#|   There are essentially infinite options but generally a relatively simple one will
#|   work well enough for your data.
#| message: false
#| echo: false
draw_gomp_sigma <- function(x) {
  return(22 * exp(-9 * exp(-0.27 * x)))
...
#| echo: true
#| fig.alt: >
#|   Plots showing default lognormal prior distributions from growthSS along with
#|   growth data simulated from each set of priors
...
#| eval: false
ss_spline <- growthSS(
  model = "gompertz", form = y ~ time | id / group, sigma = "spline", df = simdf,
  start = list("A" = 130, "B" = 10, "C" = 0.5)
)
fit_spline <- fitGrowth(ss_spline,
  cores = 2,
...
#| eval: false
#| fig.alt: >
#|   Posterior Distribution of our growth model, shown with credible intervals ranging from 98% to 1%.
...
#| eval: false
#| fig.alt: >
#|   Posterior distribution of our growth model, forecasting time that was not part of
#|   the observed data.
...
#| eval: false
#| fig.alt: >
#|   Posterior distribution of a spline growth model, forecasting time that was not part of
#|   the observed data. In this case the splines start to fail dramatically.
...
#| echo: true
#| fig.alt: >
#|   Posterior distributions of a hypothesis test of a 5% effect size.
...
![Example changepoint model](intPlusLinear_heteroskedasticIntPlusLinear_fitGrowth.png)
...
#| fig.alt: >
#|   Example growth model of a count phenotype. 
...
#| eval: false
#| echo: false
ggsave("~/pcvr/tutorials/pcvrTutorial_advancedGrowthModeling/exCountModel.png",
  plot,
  width = 6, height = 4, dpi = 300, bg = "#ffffff"
)
```
...
#| fig.alt: >
#|   Example hierarchical growth model 
simdf <- growthSim(
  "logistic",
  n = 20, t = 25,
  params = list("A" = c(200, 160), "B" = c(13, 11), "C" = c(3, 3.5))
...
#| eval: false
#| echo: false
ggsave("~/pcvr/tutorials/pcvrTutorial_advancedGrowthModeling/exHierarchyModel.png",
  plot,
  width = 6, height = 4, dpi = 300, bg = "#ffffff"
)
```
