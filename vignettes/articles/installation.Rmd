...
With `devtools` installed we can use `install_github` to install a more frequently updated version of `pcvr` from the Danforth center github account. By default this will install package dependencies and will not build the bellwether vignette. See the \textbf{Optional Extras} and \textbf{Bellwether Vignette} sections in this document for examples of different installation options.
