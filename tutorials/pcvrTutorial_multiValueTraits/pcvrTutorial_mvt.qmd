...
## What are multi-value traits? {.smaller}
...
There are many ways to describe color and many indices that can be calculated from those colors.

Since each pixel has a color we describe the entire image using a histogram of those colors.

![Wikipedia HSV Color Space](HSV_color_solid_cylinder_saturation_gray.png)


## Multi value trait format {.smaller}

PlantCV returns multi-value traits as histograms in long format

```{r}
#dput(hue_long[3641:3660,18:21 ])
structure(list(sample = c("default", "default", "default", "default", 
"default", "default", "default", "default", "default", "default", 
"default", "default", "default", "default", "default"), trait = c("hue_frequencies", 
"hue_frequencies", "hue_frequencies", "hue_frequencies", "hue_frequencies", 
"hue_frequencies", "hue_frequencies", "hue_frequencies", "hue_frequencies", 
"hue_frequencies", "hue_frequencies", "hue_frequencies", "hue_frequencies", 
"hue_frequencies", "hue_frequencies"), value = c(0.805186656906828, 
...
5.53347973090732, 5.05245913067726), label = c(81L, 83L, 85L, 
...
```{r}
structure(list(hue_frequencies.45 = c(0.0154487872701993, 0, 
0, 0, 0), hue_frequencies.47 = c(0, 0, 0, 0.0749063670411985, 
0), hue_frequencies.49 = c(0.0308975745403986, 0.37593984962406, 
0.0774593338497289, 0, 0), hue_frequencies.51 = c(0, 0, 0, 0, 
0.0657894736842105)), row.names = 100:104, class = "data.frame")
```
...
## Functions for multi-value trait analysis  {.smaller}
...
We can make joyplots and optionally perform some tests using `pcv.joyplot`

```{r}
#| echo: true
#| warning: false
(p<-pcv.joyplot(sim_df, index = "sim", group=c("group"), method=NULL, compare=NULL))
```
## `pcv.joyplot` {.smaller}
...
We can also run KS tests or compare PDFs, both of which also return a dataframe.

```{r}
#| echo: true
p2<-pcv.joyplot(sim_df, index = "sim", group=c("group"),
                method="pdf", hypothesis = "unequal", compare= NULL)
p2$stats
```

## `pcv.emd` {.smaller}

Sometimes pairwise comparisons do not satisfactorily answer a research question or a more specific hypothesis should be tested than those possible through `pcv.joyplot`. 

For those situations one option is to use earth mover's distance.

## Earth Mover's Distance {.smaller}
...
p2 <- ggplot(df[df$y=="original",], aes(x = x))+
  ...
  scale_x_continuous(breaks=c(1:10))+
  scale_y_continuous(limits=c(0,15))+
  geom_rect(data = data.frame(x=1), aes(xmin=4.5, xmax=5.5, ymin=14, ymax=15),
            fill="blue",color="white", alpha=0.25)+
  geom_rect(data = data.frame(x=1), aes(xmin=5.5, xmax=6.5, ymin=6, ymax=7),
...
#| output: false
#| echo: true
res <- conjugate(s1, s2, method = "lognormal",
