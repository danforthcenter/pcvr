...
Image of a corn plant analyzed using plantcv

Here our segmentation easily shows area, height, and other numeric
phenotypes. To describe the color of this plant then we need a more
complex phenotype.

There are many ways to describe color, many wavelengths to measure color
in, and many indices that can be calculated from those wavelengths.

Since each pixel has a color we describe the entire image using a
histogram of those values

![Wikipedia HSV Color Space
Graphic](HSV_color_solid_cylinder_saturation_gray.png)

Wikipedia HSV Color Space Graphic
...
In `pcvr` you can choose to read multi value traits in wide format with
`mode="wide"` in `read.pcv`. Note that for output from older versions of
`PlantCV` this sort of functionality may be helpful, but any versions
...
distributions.](pcvrTutorial_mvt_files/figure-html/unnamed-chunk-6-1.png)
...
![Joyplot of simulated Earth Mover's Distance based on several
probability
distributions.](pcvrTutorial_mvt_files/figure-html/unnamed-chunk-7-1.png)

The plot output is a `ggplot` object so we can change it to match our
...
![Joyplot of simulated Earth Mover's Distance based on several
probability distributions colored by
Hue.](pcvrTutorial_mvt_files/figure-html/unnamed-chunk-8-1.png)

## `pcv.emd`

Sometimes the multi-value trait histograms will not follow a unimodal
...
EMD measures the amount of work needed to change one histogram into
another.

EMD then can be used to quantify the difference in color histograms.

Here the EMD would be 0.01 since only 1 count has to move 1 space out of
...
![Example of how Earth Mover's Distance calculates differences in
histograms.](pcvrTutorial_mvt_files/figure-html/unnamed-chunk-9-1.png)
...
If `mat=TRUE` then a distance matrix is returned, otherwise a long
...
Looking at the plot we can see that there are obvious differences
...
![Heatmap of simulated Earth Mover's Distance based on several
probability
distributions.](pcvrTutorial_mvt_files/figure-html/unnamed-chunk-11-1.png)

## `pcv.net`

With non-matrix `pcv.emd` output we can make a network to better
understand the color similarities in our data. Here we include a numeric
filter which removes edges with an EMD below that value.

Note that the default behavior changes EMD from a distance to a
dissimilarity.
...
Our results show very clear clustering and highlight that the uniform
...
![Network of simulated Earth Mover's Distance based on several
probability
distributions.](pcvrTutorial_mvt_files/figure-html/unnamed-chunk-14-1.png)

Here we specify our filter as a character, which is interpreted as a
...
![Network of simulated Earth Mover's Distance based on several
probability distributions with more edges
kept.](pcvrTutorial_mvt_files/figure-html/unnamed-chunk-16-1.png)

## `conjugate`

Sometimes your distributions follow a parameterized distribution in
which case more robust comparisons are possible.

The `conjugate` function can compare “t” (gaussian means), “gaussian”,
“beta”, “lognormal”, “dirichlet”, and “dirichlet2” distributions of
multi-value traits.

## `conjugate`

Multi-value traits can be provided to `conjugate` as matrices. Here we
...
Now we can run `conjugate` on our samples. Here we use assume a
...
```
    ...
    ## 
    ## Sample 1 Prior Normal(mu = 10, sd = 5)
    ##  Posterior Normal(mu = 4.141, sd = 0.701, lognormal_sigma = 0.143)
    ## Sample 2 Prior Normal(mu = 10, sd = 5)
    ##  Posterior Normal(mu = 3.168, sd = 0.778, lognormal_sigma = 0.346)
    ## 
    ## Posterior probability that S1 is equal to S2 = 50.909%
    ## 
...
![Plot of conjugate output using multi-value
traits](pcvrTutorial_mvt_files/figure-html/unnamed-chunk-18-1.png)

We can also perform region of practical equivalence tests by specifying
a `rope_range`.
...
```
    ...
    ## Posterior probability that S1 is equal to S2 = 50.909%
    ## 
    ## Probability of the difference between Mu parameters being within [-10:10] using a 89% Credible Interval is 100% with an average difference of 0.974
    ## 
    ## 
    ##      HDE_1 HDI_1_low HDI_1_high   HDE_2 HDI_2_low HDI_2_high   hyp post.prob
    ## 1 4.140941  3.021195   5.260687 3.16762  1.924214   4.411025 equal 0.5090941
...
![Plot of conjugate output using multi-value
traits](pcvrTutorial_mvt_files/figure-html/unnamed-chunk-19-1.png)
...
```
    ...
    ## 1 equal 0.5090941 0.9738328   -0.6765613      2.694569         1
