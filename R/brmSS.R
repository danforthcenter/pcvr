...
.brmSS <- function(model, form, sigma = NULL, df, priors = NULL) {
  ...
  if (grepl("\\+", model)) {
    chngptHelperList <- .brmsChangePointHelper(model, x, y, group, dpar = FALSE, nTimes = nTimes,
    ...
    formRes <- brmsFormFun(x, y, group, dpar = FALSE, nTimes = nTimes,
    ...
    if (any(methods::is(priors, "brmsprior"))) {
      ...
      if (is.numeric(priors)) { # priors = c(130, 12, 3)
        if (length(priors) == length(pars)) {
          ...
          stop(paste0("`priors` is length ", length(priors), " while the specified model requires ",
