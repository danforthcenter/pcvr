...
#' 
#' Note that using GAMs will be less useful than fitting parameterized models as supported by \code{growthSS} and \code{fitGrowth}
#' for common applications in plant phenotyping. 
#' 
#' @param model A model fit with smooth terms by \code{mgcv::gam}
#' @param newdata A data.frame of new data to use to make predictions. If this is left NULL (the default) then
#' an attempt is made to make newdata using the first smooth term in the formula. See examples for guidance on 
#' making appropriate newdata
#' @param g1 A character string for the level of byVar to use as the first group to compare, if plot=TRUE then 
#' this will be shown in blue.
#' @param g2 The second group to compare (comparison will be g1 - g2). If plot=TRUE then this will be shown in red.
...
#' @examples 
#' 
#' ex<-pcvr::growthSim("logistic", n=20, t=25,
...
gam_diff <- function(model, newdata=NULL, g1, g2, byVar=NULL, smoothVar=NULL, cis = seq(0.05, 0.95, 0.05),
    ...
    if(is.null(newdata)){
      newdata <- expand.grid(x = seq(min(mdf[[xTerm]]), max(mdf[[xTerm]]),
  ...
  cis_diff_df <- do.call(cbind, lapply(cis, function(ci){
    ...
    upr <- dif + (crit * se)
    lwr <- dif - (crit * se)
    setNames(data.frame(upr, lwr), paste0(c("Q_diff_", "Q_diff_"), round(c( 1-((1-ci)/2) , 1-(1-((1-ci)/2))), 3) ))
  }))
  ...
  if(plot){
    ...
    cis_g1_df <- do.call(cbind, lapply(cis, function(ci){
      ...
      setNames(data.frame(upr, lwr), paste0(c("Q_g1_", "Q_g1_"), round(c( 1-((1-ci)/2) , 1-(1-((1-ci)/2))), 3) ))
    }))
    ...
    cis_g2_df <- do.call(cbind, lapply(cis, function(ci){
      ...
      setNames(data.frame(upr, lwr), paste0(c("Q_g2_", "Q_g2_"), round(c( 1-((1-ci)/2) , 1-(1-((1-ci)/2))), 3) ))
    }))
  ...
  if(plot){
    ...
    p_model <- .plot_gam_diff(out_df, name_pattern = "Q_g1_",
                                 name_pattern2 = "Q_g2_")
    if(patch){
      patchPlot <- p_model/p_diff + patchwork::plot_layout(design = c(area(1,1,4,6), area(5,1,6,6)) )
...
.plot_gam_diff<-function(df, name_pattern = "Q_diff_", name_pattern2 = NULL){
  ...
  if(is.null(name_pattern2)){
    lineLayer <- list(ggplot2::geom_line(ggplot2::aes(y = .data[["mu"]])),
  ...
  splinePlot <- ggplot2::ggplot(df, aes(x=.data[[x]]))+
    lapply(length(cis):1, function(i){
      ci = cis[i]
      ggplot2::geom_ribbon(ggplot2::aes(ymin = .data[[paste0(name_pattern, 1-(1-((1-ci)/2)) )]], 
  ...
  if(!is.null(name_pattern2)){
    ...
    splinePlot <- splinePlot +
      lapply(length(cis):1, function(i){
        ci = cis[i]
        ggplot2::geom_ribbon(ggplot2::aes(ymin = .data[[paste0(name_pattern2, 1-(1-((1-ci)/2)) )]], 
