...
#' Additional arguments are documented in \link{fitGrowthBrm}, \link{fitGrowthNlme},
#' \link{fitGrowthNls}, \link{fitGrowthNlrq}, \link{fitGrowthMgcv}, \link{fitGrowthSurvReg},
#' \link{fitGrowthFlexSurvReg}.
...
fitGrowth <- function(ss, ...) {
  if (ss$model != "gam") {
    ...
    if (ss$type == "brms") {
      ...
      fit <- fitGrowthNlmeGam(ss, ...)
    } else if (ss$type == "nls") {
      fit <- fitGrowthNlsGam(ss, ...)
    } else if (ss$type == "nlrq") {
      fit <- fitGrowthNlrqGam(ss, ...)
...
fitGrowthBrm <- function(ss, iter = 2000, cores = getOption("mc.cores", 1), chains = 4, prior = NULL,
