#' Ease of use wrapper function for fitting various growth models specified by \code{growthSS}
#' 
...
#' Additional arguments are documented in \link{fitGrowthBrm}, \link{fitGrowthNlme}, \link{fitGrowthNls}, \link{fitGrowthNlrq},
#' \link{fitGrowthMgcv}, \link{fitGrowthSurvReg}, \link{fitGrowthFlexSurvReg}.
...
fitGrowth<-function(ss, ...){
  ...
  if(ss$model !="gam"){
    if(ss$type=="brms"){
      ...
      fit <- fitGrowthNlrq(ss, ...)
    } else if(ss$type =="survreg"){
      fit <- fitGrowthSurvReg(ss, ...)
    } else if(ss$type == "flexsurv"){
      fit <- fitGrowthFlexSurvReg(ss, ...)
    }
  } else{
    if(ss$type=="brms"){
      fit <- fitGrowthBrm(ss, ...)
    } else if(ss$type=="nlme"){
      fit <- fitGrowthNlme_gam(ss, ...)
    } else if(ss$type =="nls"){
      fit <- fitGrowthNls_gam(ss, ...)
    } else if(ss$type =="nlrq"){
      fit <- fitGrowthNlrq_gam(ss, ...)
    } else if(ss$type == "mgcv"){
      fit <- fitGrowthMgcv(ss, ...)
    } 
  }
  return(fit)
}
...
fitGrowthBrm <- function(ss, iter = 2000, cores = getOption("mc.cores",1), chains = 4, prior=NULL, backend = "cmdstanr", silent=0, ...){
  if(!"prior" %in% names(ss) && is.null(prior)){
    warning("No prior was specified. Flat priors will be used, this is likely to cause problems in model fitting and yield less accurate results. If you are fitting a GAM then ignore this warning.")
...
fitGrowthNls <- function(ss, ...){
  fit<-stats::nls(formula = ss[["formula"]],
                  data = ss[["df"]],
                  start = ss[["start"]], ...)
  return(fit)
}
...
fitGrowthNls_gam <- function(ss, ...){
  fit<-stats::lm(formula = ss[["formula"]],
                  data = ss[["df"]], ...)
  return(fit)
}
...
#' 
#' @param ss A list generated by \code{growthSS}.
#' @param cores Optionally specify how many cores to run in parallel if ss$taus is >1L. Defaults to 1 if mc.cores option is not set globally.
