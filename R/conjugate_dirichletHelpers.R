...
#' This is agnostic to the appearance of the "curve" shown by the histogram at the expense of not
#' providing single HDE and HDI values. HDE and HDI for the dirichlet are vectors of length n = n_bins.
#' Note that by default the HDE and HDI for each sample are not returned. Those can be estimated as
#' shown in examples.
#'
#' Note that this returns a data table for convenience in printing the list columns if rope_range is
#' specified.
#'
#'
#' For brevity the HDE and HDI of the samples are not returned when
#' using the dirichlet method. The "dirichlet" method does not compute an HDI and only compares how much
#' of the posterior distribution (sample 1 - sample 2) is within the ROPE interval.
#' The "dirichlet2" method does make an HDI and run normal ROPE tests per each bin using the alpha
#' vector from each sample to simulate draws from the dirichlet. Bear in mind that dirichlet is
#' sensitive to the total number of counts in the data. If the mean vector
...
#' @details
#' \itemize{
#' \item{\strong{"dirichlet": } \code{priors = list(alpha = rep(1, ncol(s1),
#' prec_a = 0.5, prec_b = 0.5 ))},
...
#' The column names should include a number representing the "bin".
#' @param s2 An optional second sample of the same form as s2.
#' @examples
#' if (FALSE) {
#'   makeMvLn <- function(bins = 500, mu_log, sigma_log) {
#'     setNames(
#'       data.frame(matrix(hist(rlnorm(2000, mu_log, sigma_log),
#'         breaks = seq(1, bins, 5), plot = FALSE
#'       )$counts, nrow = 1)),
...
#'   out <- .conj_diri_mv_2(s1, s2,
#'     priors = NULL, plot = TRUE, rope_range = c(-0.1, 0.1),
...
#'
#'   if (FALSE) {
#'     HDI <- as.data.frame(bayestestR::hdi(as.data.frame(extraDistr::rdirichlet(10000, alpha_vector)),
#'       ci = cred.int.level
#'     ))
#'     HDE_pre <- as.data.frame(bayestestR::hdi(as.data.frame(extraDistr::rdirichlet(
#'       10000,
#'       alpha_vector
#'     )), ci = 0.001))
#'     HDE <- data.frame(
#'       "Parameter" = HDE_pre[["Parameter"]],
...
#'     s1 <- mv_ln[1:30, ]
#'     s2 <- mv_ln[31:60, ]
#'     diri_ex_1 <- conjugate(s1, s2,
#'       method = "dirichlet", priors = NULL, plot = FALSE,
#'       rope_range = c(-0.025, 0.025), rope_ci = 0.89,
#'       cred.int.level = 0.89, hypothesis = "equal"
#'     )
#'
#'     diri_ex_2 <- conjugate(s1, s2,
#'       method = "dirichlet2", priors = NULL, plot = FALSE,
#'       rope_range = c(-0.025, 0.025), rope_ci = 0.89,
#'       cred.int.level = 0.89, hypothesis = "equal"
#'     )
#'   }
...
.conj_diri_mv_2 <- function(s1 = NULL, priors = NULL,
  ...
  if (is.null(priors)) {
    priors <- list(
      alpha = rep(1, ncol(s1)), # alpha vector priors
  ...
  if (!is.null(s1)) {
    ...
    if (plot) {
      out$plot_df <- data.frame(
        "bin" = seq_along(s1), "prob" = mean1,
        "sample" = rep("Sample 1", ncol(s1))
      )
    }
  } else {
    stop("s1 is required")
...
.diri2_rope <- function(sample_results, rope_range = c(-0.1, 0.1), rope_ci = 0.89, plot) {
  ...
  rope_probs <- lapply(seq_along(posterior), function(bin) {
  ...
  hdiRope <- as.data.frame(bayestestR::hdi(as.data.frame(posterior), ci = rope_ci))
  hdeRopePre <- as.data.frame(bayestestR::hdi(as.data.frame(posterior), ci = 0.001))
  hdeRope <- data.frame(
    "Parameter" = hdeRopePre[["Parameter"]],
    "HDE" = rowMeans(hdeRopePre[, c("CI_low", "CI_high")])
  )
  rope_res <- list()
  rope_res$summary <- hdeRope
  ...
  out$summary <- cbind(out$summary, data.frame(
    "HDE_rope" = list(1), "HDI_rope" = list(1),
    "rope_probs" = list(1), "mean_rope_prob" = mean_rope_prob
  ))
  out$summary <- data.table::as.data.table(out$summary)
  out$summary$HDE_rope <- list(hdeRope)
  out$summary$HDI_rope <- list(hdiRope)
...
.diri1_rope <- function(sample_results, rope_range = c(-0.1, 0.1), rope_ci = 0.89, plot) {
  ...
  if (!is.null(rope_range)) {
    if (length(rope_range) == 2) {
      ...
      rope.prob <- sum(apply(cbind(norm_abs_mean_diff, max(rope_range)),
                             MARGIN = 1, function(i) min(i)), na.rm = TRUE)

      if (plot) {
        rope_res$rope_df <- data.frame(bin = seq_along(norm_mean_diff), CI = "1")
        rope_res$rope_df$CI_low <- unlist(lapply(seq_along(norm_mean_diff), function(i) {
          min(norm_mean_diff[[i]], 0)
        }))
        rope_res$rope_df$CI_high <- unlist(lapply(seq_along(norm_mean_diff), function(i) {
          max(norm_mean_diff[[i]], 0)
        }))
      }

      rope_res$summary <- data.frame(
        "HDE_rope" = NA, "HDI_rope" = NA, "rope_probs" = NA,
        "mean_rope_prob" = rope.prob
      )
    } else {
...
.conj_diri_plot <- function(res, s2, rope_range, rope_ci) {
  ...
  p <- ggplot2::ggplot(res$plot_df, ggplot2::aes(x = .data$bin, y = .data$prob)) +
    ggplot2::geom_col(
      data = res$plot_df[res$plot_df$sample == "Sample 1", ], fill = "red",
  ...
  if (!is.null(s2)) {
    ...
    p <- ggplot2::ggplot(res$plot_df, ggplot2::aes(
      x = .data$bin, y = .data$prob,
      fill = .data$sample
    )) +
      ggplot2::geom_col(alpha = 0.75, position = "identity") +
      ggplot2::labs(
        y = "Density", title = "Distribution of Samples",
  ...
  if (length(rope_range) == 2) {
    ...
    #* this makes the rope probability look incorrect because it does not accurately show the
    #* distribution,
    ...
    rope_plot <- ggplot2::ggplot(rdf, ggplot2::aes(
      xmin = .data$bin - rect_width, xmax = .data$bin + rect_width,
      ymin = .data$CI_low, ymax = .data$CI_high
    )) +
      lapply(seq_along(cis), function(i) {
        ggplot2::geom_rect(
          data = rdf[rdf$CI == cis[[i]], ],
          ggplot2::aes(fill = as.character(cis[[i]]))
        )
      }) +
      ggplot2::geom_rect(
        data = data.frame(x = 0, y = 0), xmin = -Inf, xmax = Inf,
