...
.parsePcvrForm <- function(form, df=NULL){
  ...
  USEGROUP = FALSE
  if(grepl("\\|", x) & grepl("\\/",x)){ # Y ~ X|id/group
    ...
    USEINDIVIDUAL = TRUE
    if(!is.null(df)){
      if(length(unique(df[[group]]))==1){USEGROUP=FALSE}else{USEGROUP=TRUE} # if there is only one group then ignore grouping
    }
  } else if (grepl("\\|", x)){ # Y ~ X|group
  ...
  return(list("y" = y, "x" = x, "individual" = individual, "group" = group,
