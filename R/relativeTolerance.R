...
#'               
#' sv<-bw.time(sv, plantingDelay = 0, phenotype="area_pixels",
  ...
  datsp<-split(x=df, f=group_no_control_factor)
    ...
    d2<-do.call(rbind, lapply(datsp, function(d){
      d_res<-do.call(rbind, lapply(phenotypes, function(pheno){
        ...
        ctrl_se <- sd(d[d[[control]]==controlGroup, pheno])/length(d[d[[control]]==controlGroup, pheno])
        pheno_res<-do.call(rbind, lapply( setdiff(unique(d[[control]]), controlGroup), function(cg){
          ...
          setNames(data.frame(cg, d[1,group_no_control], pheno, mu_rel, se_rel, mu_eg, se_eg, ctrl_mean, ctrl_se),
                   c(control, group_no_control, "phenotype", "mu_rel", "se_rel", "mu_trt", "se_trt", "mu_control", "se_control"))
          }))
        pheno_res
      }))
      d_res
      ...
      d2<-do.call(rbind, lapply(datsp, function(d){
        d_res<-do.call(rbind, lapply(phenotypes, function(pheno){
          ...
          pheno_res<-do.call(rbind, lapply( setdiff(unique(d[[control]]), controlGroup), function(cg){
            ...
            se_eg <- sd(d[d[[control]]==cg, pheno])/length(d[d[[control]]==cg & d[[traitCol]]==pheno, valueCol])
            #* relative center and propogated error
            mu_rel <- mu_eg/ctrl_mean
            se_rel <- sqrt( ((se_eg / mu_eg)^2) + ((ctrl_se / ctrl_mean)^2) ) * mu_rel
            setNames(data.frame(cg, d[1,group_no_control], pheno, mu_rel, se_rel, mu_eg, se_eg, ctrl_mean, ctrl_se),
