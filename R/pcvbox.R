...
#'  Defaults to "trait".
#' @param value If the data is in long format then `value` is used for numeric values
#' for the phenotype Y. Defaults to "value".
...
#' }
#' ## End(Not run)
pcvBox <- function(df = df, x = "treatment", y = "area_pixels", fill = NULL,
                   compare = FALSE, trait = "trait", value = "value",
                   method = "t.test", showPoints = FALSE, ...) {
  ...
  boxLayer <- if (!showPoints) {
    ggplot2::geom_boxplot(ggplot2::aes(fill = .data[[fill]]), ...)
  } else {
    ggplot2::geom_boxplot(ggplot2::aes(fill = .data[[fill]]), outlier.shape = NA, ...)
  }
  pointLayer <- if (showPoints) {
