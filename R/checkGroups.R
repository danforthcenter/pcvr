...
#' @return If there are duplicates in the grouping then this will return a message with code to start checking the duplicates in your data.
#' 
#' @examples 
#' 
#' if(FALSE){ 
...
checkGroups <- function(df, group){
  ...
  if(any(tab>1)){
    ...
    dupString<-paste0(dataname,"[duplicated(interaction(", paste( paste0(dataname,"$",c(group)), collapse=", "), ")),]")
    firstDup<-paste0(dataname,"[interaction(", paste( paste0(dataname,"$",c(group)), collapse=", "), ")=='",nms[1],"',]")
    eval(parse(text=dupString))
    w<-paste0("There are ", length(nms), " observations that are not uniquely identified.",
