...
#' ss1 <- growthSS(model = "survival weibull", form = y > 100 ~ time|id/group, df = df, start=c(0,5))
...
#' ss2 <- growthSS(model = "survival binomial", form = y > 100 ~ time|id/group, df = df, start=c(-4,3))
...
brmSurvPlot <- function(fit, form, df=NULL, groups = NULL, timeRange = NULL, facetGroups=TRUE, groupFill=FALSE, virMaps = c("plasma")){
  family = as.character(fit$family)[1]
  
  if(family=="weibull"){
    p <- .weibull_brmSurvPlot(fit = fit, form=form, df=df, groups = groups, timeRange = timeRange, facetGroups=facetGroups,
                         groupFill=groupFill, virMaps = c("plasma"))
  } else if(family=="binomial"){
    p <- .binomial_brmSurvPlot(fit = fit, form=form, df=df, groups = groups, timeRange = timeRange, facetGroups=facetGroups,
                         groupFill=groupFill, virMaps = c("plasma"))
  }
  return(p)
}
...
.binomial_brmSurvPlot<-function(fit, form, df=NULL, groups = NULL, timeRange = NULL, facetGroups=TRUE, groupFill=FALSE, virMaps = c("plasma")){
  ...
  # apply(s_hat,2,summary)
  #* was taking cumprod of all columns but columns need to be by group, fix it.
  s_hat <- as.data.frame(
    do.call(rbind, lapply(groups, function(grp){
      s_hat_grp <- s_hat[, grepl(paste0(group, grp, "$"), colnames(s_hat))]
      x<-do.call(cbind, lapply(1:ncol(s_hat_grp), function(j){
        do.call(rbind, lapply(1:nrow(s_hat_grp), function(i){
...
.weibull_brmSurvPlot<-function(fit, form, df=NULL, groups = NULL, timeRange = NULL, facetGroups=TRUE, groupFill=FALSE, virMaps = c("plasma")){
  #* `Transform draws`
  fitData <- fit$data
  fdf <- as.data.frame(fit)
  mu_cols <- colnames(fdf)[grepl("b_", colnames(fdf))]
  scales <- do.call(cbind, lapply(mu_cols, function(col){
    scale = exp(fdf[[col]]) / (gamma(1 + (1 / fdf$shape )))
  }))
  ...
  if(!is.null(df)){
    km_df <- do.call(rbind, lapply(groups, function(grp){
      ...
      do.call(rbind, lapply(timeRange, function(ti){
        ...
        data.frame(group = grp, time = ti, events = sum_events, at_risk = n_at_risk, surv_pct = surv_pct)
      }))
    }))
