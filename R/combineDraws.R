...
#' 
#' @description Helper function for binding draws from several \code{brms} models to make a data.frame for use with
#' \code{brms::hypothesis()}. This will also check that the draws are comparable using basic model metrics.
#' 
#' @param ... Some number of brmsfit objects and/or dataframes of draws (should generally be the same type of model fit to different data)
#' @param message Logical, should messages about possible problems be printed? Default is TRUE. This will warn if models may
#' not have converged, if there are different numbers of draws in the objects, or if models have different formulations.
#' @keywords brms
...
#' params = list("A"=c(200,160, 220, 200, 140, 300),
#'               "B"=c(13, 11, 10, 9, 16, 12),
#'               "C"=c(3, 3.5, 3.2, 2.8, 3.3, 2.5)))
#' ss_ab<-growthSS(model = "logistic", form=y~time|id/group,
#'                 sigma="logistic", df=simdf[simdf$group %in% c("a","b"),],
#'                 start = list("A"=130, "B"=12, "C"=3, "subA"=15, "subB"=10, "subC"=3), type="brms")
#' 
#' ss_cd<-growthSS(model = "logistic", form=y~time|id/group,
#'                 sigma="logistic", df=simdf[simdf$group %in% c("c","d"),],
#'                 start = list("A"=130, "B"=12, "C"=3, "subA"=15, "subB"=10, "subC"=3), type="brms")
#' 
#' ss_ef<-growthSS(model = "logistic", form=y~time|id/group,
#'                 sigma="logistic", df=simdf[simdf$group %in% c("e","f"),],
#'                 start = list("A"=130, "B"=12, "C"=3, "subA"=15, "subB"=10, "subC"=3), type="brms")
#' ss_ef2<-growthSS(model = "gompertz", form=y~time|id/group,
#'                  sigma="logistic", df=simdf[simdf$group %in% c("e","f"),],
...
combineDraws <- function(..., message = TRUE){
  ...
  models <- objects[unlist(lapply(objects, function(m){methods::is(m, "brmsfit")}))]
  model_names <- obj_names[unlist(lapply(objects, function(m){methods::is(m, "brmsfit")}))]
  if(length(model_names)==1){models <- list(models)}
  supplied_draw_dfs <- objects[unlist(lapply(objects, function(m){methods::is(m, "data.frame")}))]
  df_names <- obj_names[unlist(lapply(objects, function(m){methods::is(m, "data.frame")}))]
  ...
  new_draws <- do.call(cbind, lapply(1:length(models), function(i){
      m <- models[[i]]
      mn <- model_names[[i]]
      d<-as.data.frame(m)
      ...
      if(message){
        ...
        if(any(rhats>1.05)){
          message(paste0(mn, " has Rhat values >1.05 for some model parameters. See ?barg for possible improvements.\n"))
        }
      }
      
      if(nrow(draws)<limit_size){
        if(message){
          message(paste0(mn, " has fewer than ", limit_size, " draws and will be padded with ", limit_size-nrow(draws), " NAs\n"))
        }
  ...
  if(length(supplied_draw_dfs)>0){
    ...
    supplied_draw_dfs <- lapply(1:length(supplied_draw_dfs), function(i){
      df <- supplied_draw_dfs[[i]]
      dn <- df_names[[i]]
      if(nrow(df)<limit_size){
        if(message){
          message(paste0(dn, " has fewer than ", limit_size, " draws and will be padded with ", limit_size-nrow(df), " NAs\n"))
        }
