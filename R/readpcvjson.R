...
read.pcv.jsn<-function(file, dtSuffix = "datatype", valSuffix = "value", labSuffix = "label", output = "sv"){
  ...
  if(!all(c(dtSuffix, valSuffix, labSuffix) %in% fieldTypes)){
    ...
    stop(paste0("'",names[which(!names %in% fieldTypes)], "' not in column suffixes. Suffixes in data are: ", paste(fieldTypes, collapse=", ")))
  }
  dt<-paste0(".", dtSuffix, "$")
  ...
  if(!all(output %in% c("sv", "mv", "all"))){
    jsn<-jsn[,grepl(paste0(c(paste0("[.]",output,"[.]"), "metadata"), collapse="|"), colnames(jsn))]
    output = "all"
  }
  ...
  datatypes<-unlist(lapply(dtcols, function(c){ strsplit(unique(jsn[!is.na(jsn[,c]),c]),"[']")[[1]][2] } ))
  ...
  listCols<-if(is.null(dtl[["list"]])){"NOLISTCOLUMNS"} else {dtl[["list"]]}
  tupleCols<-if(is.null(dtl[["tuple"]])){"NOTUPLECOLUMNS"} else {dtl[["tuple"]]}
  sv<-sub[,!grepl(paste0(listCols,collapse="|"), colnames(sub))]
  sv<-sv[,!grepl("metadata", colnames(sv))]
  if(length(tupleCols)>0){tv<-sv[,grepl(paste0(tupleCols,collapse="|"), colnames(sv))]}
  ...
  if(output %in% c("sv", "all")){
    ...
    tupPhenos<-unique(gsub(paste0(lab, val, collapse="|"),"",colnames(tv)))
    
    tv<-do.call(cbind,lapply(colnames(tv)[grepl(val,colnames(tv))], function(c){
      l<-tupL[[c]]
      labs<-unique(unlist(tv[[gsub(val,".label",c)]]))
      tupleExpanded<-data.frame(do.call(rbind,lapply(tv[[c]], function(c) if(any(is.null(c))||any(is.na(c))){list(rep(NULL,l))} else{ c} )))
      colnames(tupleExpanded)<-paste0(gsub(val,"",c), ".", labs)
      tupleExpanded
    }))
    if(nrow(tv) == nrow(sv)){sv<-cbind(sv,tv)}
    }
  }
  if(output %in% c("mv", "all")){
    ...
    mL<-lapply(mv, function(li){
      li<-li[!is.null(li) & !is.na(li)]
      length(unlist(li[1])) 
    })
    mPhenos<-unique(gsub(paste0(lab, val, collapse="|"),"",colnames(mv)))
    
    mvL<-do.call(rbind,lapply(colnames(mv)[grepl(val,colnames(mv))], function(c){
      lng<-do.call(rbind,lapply(1:nrow(mv), function(i){
        data.frame(row = i, value = unlist(mv[1,c]), label = unlist(mv[1,gsub(val, paste0(".",labSuffix), c)]),
                   trait = gsub(val,"",c) )
      }))
      lng
    }))
  ...
  if(output == "sv"){
    ...
    ret = list()
    if(all(dim(sv)>0)){ret[["SV"]]=sv}
    if(all(dim(mv)>0)){ret[["MV"]]=mv}
